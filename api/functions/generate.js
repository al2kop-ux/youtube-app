...
exports.handler = async (event) => {
    ...
    try {
        const { topic, persona, urls } = JSON.parse(event.body);

        if (!topic) {
            return {
                statusCode: 400,
                body: JSON.stringify({ error: 'Video topic is required.' })
            };
        }
...
${sourceMaterial.trim() ? sourceMaterial : "No source material provided. Please generate the outline based on the topic alone."}
    `;
        ...
        if (!apiKey) {
            return {
                statusCode: 500,
                body: JSON.stringify({ error: 'Gemini API key is not set.' })
            };
        }
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ error: `[Generate Function Error]: ${errorMessage}` })
        };
    }
};
